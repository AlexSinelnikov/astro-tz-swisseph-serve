...
def release_lock(lock_path: Optional[str]):
    ...
    for url in [u.strip() for u in urls if u.strip()]:
        for attempt in range(1, tries+1):
            try:
                ...
                if size == 0: raise IOError("Пустой файл после скачивания")
...
def validate_zip(path: str) -> bool:
    try:
        with zipfile.ZipFile(path) as zf:
            if not zf.namelist():
                log("Архив пуст."); return False
            bad = zf.testzip()
            if bad:
                log(f"Повреждённый ZIP, проблемный файл: {bad}"); return False
        return True
    except Exception as e:
        log(f"Bad ZIP: {e}"); return False

def _list_dir_sample(root: str, n: int = 20) -> List[str]:
    try:
        names = sorted(os.listdir(root))[:n]
        return [os.path.join(root, x) for x in names]
    except Exception:
        return []

# --- регистронезависимая проверка наличия ---
def have_required(root: str, patterns_csv: str) -> Tuple[bool, Dict[str, List[str]]]:
    """
    Ищем требуемые файлы регистронезависимо:
      - сначала по паттерну в корне,
      - если пусто и паттерн без слэшей — пробуем рекурсивно (**/)
    """
    res: Dict[str, List[str]] = {}
    ok_all = True
    pats = [p.strip() for p in patterns_csv.split(",") if p.strip()]
    for pat in pats:
        # обычный glob (регистрозависимый)
        matches = glob.glob(os.path.join(root, pat))
        # если ничего не нашли — делаем регистронезависимый поиск вручную
        if not matches:
            want = pat.lower()
            ...
            for path in cand:
                ...
                # сопоставляем по простому правилу масок sepl_*.se1 / semo_*.se1 / seas_*.se1
                if want.startswith("sepl_") and want.endswith(".se1") and name.startswith("sepl_") and name.endswith(".se1"):
                    mm.append(path)
                elif want.startswith("semo_") and want.endswith(".se1") and name.startswith("semo_") and name.endswith(".se1"):
                    mm.append(path)
                elif want.startswith("seas_") and want.endswith(".se1") and name.startswith("seas_") and name.endswith(".se1"):
                    mm.append(path)
            matches = mm
        ok = len(matches) > 0
        res[pat] = matches
...
def extract_needed(zip_path: str, dst_root: str, keep_txt: bool) -> int:
    """
    Извлекаем *.se1 (и опц. *.txt) в КОРЕНЬ dst_root,
    ПРИ ЭТОМ ПРИВОДИМ ИМЕНА К нижнему регистру (чтобы маски гарантированно сработали).
    """
    ensure_dir(dst_root)
    extracted = 0
    with zipfile.ZipFile(zip_path) as zf:
        for info in zf.infolist():
            base_orig = os.path.basename(info.filename.replace("\\","/"))
            if not base_orig:
                continue
            base_low = base_orig.lower()
            if base_low.endswith(".se1") or (keep_txt and base_low.endswith(".txt")):
                target = os.path.join(dst_root, base_low)  # <-- НИЖНИЙ РЕГИСТР
                with zf.open(info) as src, open(target, "wb") as out:
                    shutil.copyfileobj(src, out, length=1024*1024)
                extracted += 1
    log(f"Извлечено файлов: {extracted}")
    # Покажем что реально лежит в каталоге
    sample = _list_dir_sample(dst_root, 15)
    if sample:
        log("Пример содержимого:", ", ".join(os.path.basename(x) for x in sample))
    return extracted
...
def main():
    ...
    # Быстрое завершение, если всё уже есть
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if expected_sha:
                ...
                if got.lower() != expected_sha.lower():
                    ...
                    if not allow_missing: raise SystemExit(5)
            ...
            if need_extract:
                ...
                if extracted == 0:
                    log("В архиве не найдено *.se1 — проверь содержимое ZIP.")

        ok, details = have_required(ephe_path, required)
        if args.set_path: log(f"swisseph search path set to: {ephe_path}")
        if ok:
            log("have_required: true")
            return
        else:
            # Добавим подсказку: что именно найдено после извлечения
            log("have_required: false")
            for k, v in details.items():
                if not v:
                    log(f"После распаковки не найден шаблон {k}. Пример содержимого: "
                        f"{', '.join(os.path.basename(x) for x in _list_dir_sample(ephe_path, 20))}")
            if allow_missing:
